...
from airgym.envs.airsim_env import AirSimDiscreteEnv
from PIL import Image


class AirSimDroneDiscreteEnv(AirSimDiscreteEnv):
    def __init__(self, ip_address, step_length, image_shape, env_config, attitude_shape):
        ...
        self.action_space = spaces.Discrete(3) # Left / Hold / Right
        # self.action_space = spaces.Discrete(5) # 2Left / Left / Hold / Right / 2Right
    ...
    def _do_action(self, action):
        timestep = 0.25
        if action == 0:
            self.drone.moveByVelocityZAsync(2,2,-2, timestep).join()
            # print("RIGHT")
        elif action == 1:
            self.drone.moveByVelocityZAsync(2,-2,-2, timestep).join()
            # print("LEFT")
        else:
            self.drone.moveByVelocityZAsync(2,0,-2, timestep).join()
            # print("HOLD")
    ...
    def transform_obs(self, response):
        ...
        # print("IMAGE")
        # print(image)
        im_final = np.array(image.convert("L"), dtype=np.uint8)
    ...
    def _get_obs(self):
        ...
        # print(obs["Linear velocity"].dtype)
    ...
    def _compute_reward(self):
        ...
        if self.state["position"].x_val > 100:
            done = 1
            reward_dist = 30
            reward = reward_dist
...
class TestEnv(AirSimDroneDiscreteEnv):
